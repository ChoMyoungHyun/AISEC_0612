...
def callModel(mname):  # 모델 불러오는 함수
    tmodel = tf.keras.models.load_model(f"{ROOT_PATH}{OS_DELIM}lotto_model{OS_DELIM}{mname}.keras")
    return tmodel

def preproc(data_set, time_step):  # 데이터 전처리 함수
    df = pd.read_csv(f"{ROOT_PATH}{OS_DELIM}last_data{OS_DELIM}{data_set}", names=DATA_LABELS)
    df["year_date"] = pd.to_datetime(df["year_date"])
    df["millis"] = df["year_date"].astype("int64") // 10 ** 9
    raw_data = np.array(df.loc[:, ["num_1", "num_2", "num_3", "num_4", "num_5", "num_6", "num_7", "millis"]])
    print(raw_data.shape)
    print("원본숫자:", raw_data[0])
    raw_data = np.array([np.concatenate([raw_data[i][:7] / (raw_data[i][7] / (10 ** 8)), [raw_data[i][7]]]) for i in
                         range(len(raw_data))])
    print("복원숫자:", recoverData(raw_data[0][:7], raw_data[0][7]))
    raw_data = raw_data[::-1]
    x_data = []
    y_data = []
    for i in range(len(raw_data) - time_step + 1):
...
def printpred(tmodel, x_data, millis_sec):  # 데이터 예측 출력 함수
    ...
    return np.round(recoverData(y_pred, millis_sec)), datetime.fromtimestamp(x_data[-1][-1][7]), datetime.fromtimestamp(millis_sec)

def upgradeModel():  # 모델 업그레이드 함수
    pass
